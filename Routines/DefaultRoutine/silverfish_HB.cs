    ...
    public class Silverfish
    {
        ...
        private void updateRealTimeInfo()
        {
            ...
            if (TritonHs.DoWeHaveWeapon)//如果我们有武器
            {
                ...
                ownWeapon.Durability = weapon.GetTag(GAME_TAG.HEALTH) - weapon.GetTag(GAME_TAG.DAMAGE) - weapon.GetTag(GAME_TAG.PREDAMAGE);
            ...
            if (TritonHs.DoesEnemyHasWeapon)//对手有武器
            {
                ...
                enemyWeapon.Durability = weapon.GetTag(GAME_TAG.HEALTH) - weapon.GetTag(GAME_TAG.DAMAGE) - weapon.GetTag(GAME_TAG.PREDAMAGE);
        ...
        private void updateHandcard(HSCard card, int controller, string cardId, int entityId, int cost, Entity entity)
        {
            ...
            if (zp >= 1)
            {
                if (controller == ownController)
                {
                    ...
                    if (card.IsWeapon) hc.addHp = card.Health - card.DefHealth;
